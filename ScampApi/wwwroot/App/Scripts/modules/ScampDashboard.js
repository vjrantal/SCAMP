function ScampDashboard($scope) { 
    ...
    this.updateSummaryPanel = function(data){
        scope.summary = {
            ...
            'pctUsed': (data.totUnitsAllocated > 0) ? Math.round((user.totUnitsUsed / data.totUnitsAllocated) * 100) : 0,
            'pctAllocated': (data.unitsBudgeted > 0) ? Math.round((user.totUnitsAllocated / data.unitsBudgeted) * 100) : 0,
